(function ($) {
  ...
  $.remodalCommon = function (options = {}) {
    ...
    if (TYRANO.kag.tmp.remodal_opening_effect_time !== undefined) {
      j_anim.setStyleMap(
        { "animation-duration": TYRANO.kag.tmp.remodal_opening_effect_time },
        "webkit"
      );
    }
    ...
    if (opening_effect && opening_effect !== "none") {
      ...
      $(document).on("opened", ".remodal", () => {
        ...
        if (TYRANO.kag.tmp.remodal_closing_effect_time !== undefined) {
          j_anim.setStyleMap(
            {
              "animation-duration": TYRANO.kag.tmp.remodal_closing_effect_time,
            },
            "webkit"
          );
        }
  ...
  $.gradientPresetMap = {
    ...
    light:
      "linear-gradient(0deg, rgba(193,245,239,1) 0%, rgba(255,255,255,1) 34%, rgba(255,255,255,1) 100%)",
    ...
    gold2:
      "linear-gradient(0deg, #b8751e 0%, #ffce08 37%, #fefeb2 47%, #fafad6 50%, #fefeb2 53%, #e1ce08 63%, #b8751e 100%)",
    silver:
      "repeating-linear-gradient(0deg, #a8c7c3 0.1em, #b6c9d1 0.2em, #e7fbff 0.3em, #c7d5d6 0.4em, #a6b2b6 0.5em)",
    silver2:
  ...
  $.fn.animateWithTyranoKeyframes = function (pm) {
    ...
    for (let i = 0; i < len; i++) {
      const anim = this[i].animate(keyframes, {
        ...
        iterations:
          pm.count === "infinite" ? Infinity : parseInt(pm.count) || Infinity,
