...
tyrano.plugin.kag.tag.text = {
  ...
  showMessage: function (message_str, is_vertical) {
    ...
    const chara_name_log = this.kag.chara.getCharaName_log();
    this.pushTextToBackLog(chara_name_log, message_str);
  ...
  pushTextToBackLog: function (chara_name, message_str) {
    ...
    if (
      ...
      const log_str = `
        <div class="backlog_chara_name">
            <div class="backlog_chara_name_text ${chara_name}">${chara_name}</div>
            <div class="backlog_text ${chara_name}">${message_str}</div>
            <div class="backlog_chara_name_bottom"></div>
        </div>`;
      ...
      const log_str = `<div class="backlog_text_noname ${chara_name}">${message_str}</div>`,
  ...
  adjustCharaFukiSize: function (j_msg_inner, chara_name, chara_obj) {
    ...
    const chara_left = parseInt(chara_obj.css("left")),
      ...
      chara_width = parseInt(chara_obj.find("img").css("width")),
    ...
    fuki_top =
      chara_top +
      fuki_top *
        (chara_height / this.kag.stat.charas[chara_name].origin_height);
    const sippo_width = parseInt(chara_fuki.sippo_width),
      sippo_height = parseInt(chara_fuki.sippo_height),
      outer_width =
        (parseInt(chara_fuki.sippo_left),
        parseInt(chara_fuki.sippo_top),
        parseInt(j_outer_message.css("width")) + sippo_width),
      outer_height = parseInt(j_outer_message.css("height")) + sippo_height;
    ...
    let fuki_right = fuki_left + outer_width,
      fuki_bottom = fuki_top + outer_height;
    const sc_width = parseInt(this.kag.config.scWidth) - 10,
      sc_height = parseInt(this.kag.config.scHeight) - 10;
    ...
    if (fuki_left <= 10) {
      const overflow_width = 10 - fuki_left;
      fuki_left = 10;
      sippo_left_offset = -overflow_width;
    }
    fuki_bottom >= sc_height && (fuki_top -= fuki_bottom - sc_height);
    fuki_top <= 10 && (fuki_top = 10);
    ...
    this.kag.updateFuki(chara_name, { sippo_left: sippo_left_offset });
  },
  adjustOthersFukiSize: function (j_msg_inner) {
    const others_style = this.kag.stat.fuki.others_style,
      ...
      fuki_max_width = others_style.max_width || def_style.width,
      fuki_left = others_style.left || def_style.left,
      fuki_top = others_style.top || def_style.top;
    if ("true" !== this.kag.stat.vertical) {
      j_msg_inner.css("height", "");
  ...
  startLipSyncWithText() {
    let chara_name = this.kag.chara.getCharaName(!0);
    if (!chara_name) return null;
    const target_parts = this.kag.chara.getLipSyncParts.call(
      this,
      chara_name,
      "text"
    );
  ...
  start: function () {
    ...
    if (1 != this.kag.stat.is_skip) {
      if (1 == this.kag.stat.is_auto) {
        ...
        setTimeout(function () {
          if (1 == that.kag.stat.is_wait_auto)
            if (1 == that.kag.tmp.is_vo_play) that.kag.tmp.is_vo_play_wait = !0;
            else {
...
tyrano.plugin.kag.tag.image = {
  ...
  start: function (pm) {
    ...
    if ("base" != pm.layer) {
      ...
      if ("true" === pm.animimg) {
        const count = $(`[src^='${strage_url}'`).length,
          url_obj = new URL(strage_url, window.location.href);
