tyrano.plugin.kag = {
  ...
  init: function () {
    ...
    if (
      ...
      !window.studio_api.ipcRenderer.sendSync("doubleCheck", {})
    ) {
  ...
  applyPatch: function (patch_path, flag_reload, call_back) {
    if (window.studio_api.fs.existsSync(patch_path)) {
      var fse = window.studio_api.fs,
        _path = window.studio_api.path,
        unzip_path = $.getUnzipPath();
      if ("asar" != unzip_path) {
        const AdmZip = window.studio_api.admzip;
        window.studio_api.path.resolve("./");
        ...
        let path = process.__dirname,
          out_path = $.localFilePath();
        if ("darwin" == process.platform) {
          alert(
            "パッチを適応するゲーム実行ファイル（.app）の場所を選択してください。"
          );
          let filenames = window.studio_api.ipcRenderer.sendSync(
        ...
        new (0, window.studio_api.admzip)(patch_path).extractAllTo(
          _path.resolve(out_path + "update_tmp/"),
          !0
        );
  ...
  chara: {
    ...
    getCharaName(convert_to_id) {
      let chara_name = "";
      if (void 0 !== this.kag.stat.current_speaker)
      ...
      convert_to_id &&
        this.kag.stat.jcharas[chara_name] &&
    ...
    getCharaName_log() {
      ...
      if (
        void 0 !== this.kag.stat.current_speaker &&
        this.kag.stat.charas[this.kag.stat.current_speaker]
      ) {
        const currentSpeaker =
          this.kag.stat.charas[this.kag.stat.current_speaker];

        chara_name = currentSpeaker.logname || this.kag.stat.current_speaker;
      }
    ...
    getLipSyncParts(name, type = "voice") {
      ...
      if (!cpm) return null;
      if ("false" === !cpm.is_show) return null;
      if (cpm.lipsync_type !== type) return null;
      const j_chara = this.kag.chara.getCharaContainer(name);
      if (0 === j_chara.length) return null;
      const target_parts = [],
        part_map = cpm._layer;
      if (part_map) {
        for (const part in part_map) {
          const part_obj = part_map[part],
            state = part_obj.current_part_id,
            part_state_obj = part_obj[state];
          if (!part_state_obj || !part_state_obj.is_lipsync_enabled) continue;
          const j_frames = j_chara.find(
              `.lipsync-frame[data-effect=${part}-${state}]`
            ),
            thresholds = part_state_obj.lip_volume;
          j_frames.length > 0 &&
            thresholds &&
            target_parts.push({
              j_frames: j_frames,
        ...
        return 0 === target_parts.length ? null : target_parts;
      }
    },
    getFrameAnimationSrcs(cpm, part, state) {
      if ("allow_storage" === state) return [];
      const state_obj = cpm._layer[part][state];
      if (!state_obj.frame_image) return [];
      const srcs = [],
        image_extensions = [
          "png",
          "jpg",
        ...
        get_image_extension = (filename) => {
          const extension = filename.split(".").pop();
          return image_extensions.includes(extension.toLowerCase())
            ? extension
            : null;
        },
        base_path = $.parseStorage(state_obj.storage, "fgimage"),
        base_extension = get_image_extension(base_path);
      state_obj.frame_image.forEach((frame_src) => {
        const frame_extension = get_image_extension(frame_src),
          hash_slash = base_path.split("/");
        let new_filename;
        new_filename =
          base_extension && !frame_extension
            ? `${frame_src}.${base_extension}`
            : frame_src;
        hash_slash.pop();
    ...
    setFrameAnimation(cpm, part, state, j_frame_base, preload_srcs) {
      ...
      if (!frame_srcs.length) return null;
      const state_obj = cpm._layer[part][state];
      let j_frames = $(j_frame_base),
        j_prev_frame = j_frame_base;
      frame_srcs.forEach((frame_src) => {
        ...
        preload_srcs && preload_srcs.push(frame_src);
      ...
      if (state_obj.lip_image) {
        ...
        state_obj.is_lipsync_enabled = !0;
    ...
    startFrameAnimation(state_obj, j_frames) {
      ...
      const frame_count = state_obj.frame_image.length + 1,
        ...
        anim = () => {
          ...
          if (0 === j_frames.eq(0).closest("html").length) return;
    ...
    restoreAllFrameAnimation() {
      ...
      $(".chara-layer-frame.base").each(function () {
        try {
          const j_frame_base = $(this),
            setting = JSON.parse(j_frame_base.attr("data-restore")),
            state_obj =
              that.kag.stat.charas[setting.chara_name]._layer[
                setting.part_name
              ][setting.state_name];
          let j_frames = $(j_frame_base);
          const j_siblings = j_frame_base.siblings(
            `.chara-layer-frame[data-event-pm=${setting.part_name}]`
          );
      ...
      $(".lipsync-frame.base").each(function () {
        try {
          const j_frame_base = $(this),
            data_effect = j_frame_base.attr("data-effect");
          j_frame_base
            .siblings(`.sub[data-effect=${data_effect}]`)
    ...
    updateLipSyncWithVoice(value, target_parts, elapsed_time) {
      target_parts.forEach((part) => {
        ...
        for (
          ;
          target_i < part.thresholds.length &&
          !(value < part.thresholds[target_i]);
          target_i++
        );
        if (target_i !== part.current_index) {
          if (part.current_index === part.thresholds.length) {
            part.max_open_mouth_time += elapsed_time;
            if (part.max_open_mouth_time < 200) return;
            part.max_open_mouth_time = 0;
          }
          target_i =
            target_i > part.current_index
              ? part.current_index + 1
              : part.current_index - 1;
  ...
  convertLang(scenario, array_s) {
    ...
    if (!this.kag.map_lang.scenes[scenario]) return array_s;
    let map_trans = this.kag.map_lang.scenes[scenario],
      map_charas = this.kag.map_lang.charas,
      is_script = !1;
    for (let i = 0; i < array_s.length; i++) {
      const tobj = array_s[i];
      if ("iscript" === tobj.name) is_script = !0;
      else if ("endscript" === tobj.name) is_script = !1;
      else if ("text" === tobj.name) {
        if (!is_script) {
          let trans_text = "";
      ...
      } else if ("chara_ptext" === tobj.name) {
        if (map_charas && map_charas[tobj.pm.name]) {
          this.kag.stat.charas[tobj.pm.name]
            ? (this.kag.stat.charas[tobj.pm.name].jname =
                map_charas[tobj.pm.name])
            : (tobj.pm.name = map_charas[tobj.pm.name]);
          array_s[i] = tobj;
        }
      } else if (map_trans.tag && map_trans.tag[tobj.name]) {
        let pm = tobj.pm;
        for (let key in pm)
          map_trans.tag[tobj.name][key] &&
            map_trans.tag[tobj.name][key][pm[key]] &&
  ...
  async loadLang(name, cb) {
    if ("default" != name)
      try {
        let lang_str = await $.loadTextSync(
          "./data/others/lang/" + name + ".json"
        );
        this.lang = name;
