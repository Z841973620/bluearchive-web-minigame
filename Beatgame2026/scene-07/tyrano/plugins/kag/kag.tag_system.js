...
tyrano.plugin.kag.tag.else = {
  ...
  start: function (pm) {
    if (0 == this.kag.getStack("if").bool) {
      ...
      this.kag.ftag.nextOrderWithTagSearch({ endif: "" }) ||
        this.kag.error("missing_endif");
    }
  },
};
...
tyrano.plugin.kag.tag.macro = {
  ...
  start: function (pm) {
    ...
    this.kag.ftag.nextOrderWithTagSearch({ endmacro: "" }) ||
...
tyrano.plugin.kag.tag.ignore = {
  ...
  start: function (pm) {
    if (this.kag.embScript(pm.exp)) {
      this.kag.ftag.nextOrderWithTagSearch({ endignore: "" }) ||
...
tyrano.plugin.kag.tag.preload = {
  vital: ["storage"],
  pm: { storage: "", wait: "false", single_use: "true", name: "" },
  start: function (pm) {
    var that = this;
...
tyrano.plugin.kag.tag.unload = {
  pm: { storage: "", name: "", all_sound: "false" },
  start: function (pm) {
...
tyrano.plugin.kag.tag.cursor = {
  ...
  start: function (pm) {
    ...
    if (pm.click_effect) {
      ...
      "function" == typeof this.kag.tmp.show_effect_callback &&
        document.body.removeEventListener(
          event_type,
          this.kag.tmp.show_effect_callback,
          { capture: !0 }
        );
  ...
  restore: function () {
    ...
    this.kag.ftag.startTag("cursor", {
      click_effect: String(
        this.kag.stat.click_effect && this.kag.stat.click_effect.is_enabled
      ),
      next: "false",
    });
  },
  showEffect: function (e) {
    ...
    const base_width = parseInt(this.kag.stat.click_effect.width) || 100,
      ...
      opacity = $.convertOpacity(this.kag.stat.click_effect.opacity) || 0.8,
  ...
  overwriteCSS: function () {
    ...
    let css_text = `\n            #remodal-cancel,\n            #remodal-confirm,\n            .button_menu,\n            .menu_item img,\n            .save_list_item {\n                cursor: ${
      this.kag.stat.current_cursor_map.pointer || "pointer"
    };\n            }\n        `;
    if (this.kag.stat.click_effect) {
      css_text += `\n                @keyframes tyrano_click_effect {\n                    from {\n                        transform: translate(-50%, -50%) scale(1);\n                    }\n                    100% {\n                        transform: translate(-50%, -50%) scale(${
        (this.kag.stat.click_effect.scale || 120) / 100
      });\n                        opacity: 0;\n                    }\n                }\n            `;
    }
...
tyrano.plugin.kag.tag.awakegame = {
  vital: [],
  pm: { variable_over: "true", sound_opt_over: "true", bgm_over: "true" },
  start: function (pm) {
...
tyrano.plugin.kag.tag.pushlog = {
  ...
  pm: { text: "", join: "false" },
  start: function (pm) {
    pm.text = `<div class="backlog_text_noname">${pm.text}</div>`;
...
tyrano.plugin.kag.tag.dialog_config = {
  ...
  init() {
    if (!this.__initialized) {
      ...
      this.j_cancel = this.j_box
        .find(".remodal-cancel")
        .attr("id", "remodal-cancel");
      this.j_ok = this.j_box
        .find(".remodal-confirm")
        .attr("id", "remodal-confirm");
  ...
  start: function (pm) {
    ...
    pm.btnwidth &&
      this.j_button.setStyle("width", $.convertLength(pm.btnwidth));
    pm.btnmargin && this.j_button.setMargin(pm.btnmargin);
    pm.btnpadding && this.j_button.setPadding(pm.btnpadding);
    pm.fontsize &&
      this.j_text.setStyle("font-size", $.convertLength(pm.fontsize));
    pm.fontbold &&
      this.j_text.setStyle("font-weight", $.convertFontWeight(pm.fontbold));
    ...
    pm.btnfontsize &&
      this.j_button.setStyle("font-size", $.convertLength(pm.btnfontsize));
    pm.btnfontbold &&
      this.j_button.setStyle(
        "font-weight",
        $.convertFontWeight(pm.btnfontbold)
      );
    pm.btnfontface && this.j_button.setStyle("font-family", pm.btnfontface);
    pm.btnfontcolor &&
      this.j_button.setStyle("color", $.convertColor(pm.btnfontcolor));
    if (pm.boxradius) {
      this.j_box.setStyle(
        "border-radius",
        $.convertLength(pm.boxradius + "px")
      );
      this.j_boxbase.setStyle(
        "border-radius",
        $.convertLength(pm.boxradius + "px")
      );
    }
    pm.boxcolor &&
      this.j_boxbase.setStyle("background-color", $.convertColor(pm.boxcolor));
    pm.boximg &&
      this.j_boxbase.setStyle(
        "background-image",
        $.convertBackgroundImage(pm.boximg, "image")
      );
    pm.boximgpos &&
      this.j_boxbase.setStyle(
        "background-position",
        $.convertBackgroundPosition(pm.boximgpos)
      );
    pm.boximgsize &&
      this.j_boxbase.setStyle(
        "background-size",
        $.convertLength(pm.boximgsize)
      );
    pm.boximgrepeat &&
      this.j_boxbase.setStyle("background-repeat", pm.boximgrepeat);
    pm.boxopacity &&
      this.j_boxbase.setStyle("opacity", $.convertOpacity(pm.boxopacity));
    pm.boxwidth &&
      this.j_box
        .setStyle("width", $.convertLength(pm.boxwidth))
        .setStyle("max-width", $.convertLength(pm.boxwidth));
    pm.boxheight &&
      this.j_box
        .setStyle("height", $.convertLength(pm.boxheight))
        .setStyle("max-height", $.convertLength(pm.boxheight));
    pm.boxpadding && this.j_box.setPadding(pm.boxpadding);
    (pm.boximg || pm.boxcolor) &&
      this.j_box.setStyleMap({ background: "none" });
    (pm.bgimg || pm.bgcolor) &&
    ...
    pm.bgcolor &&
      this.j_image.setStyle("background-color", $.convertColor(pm.bgcolor));
    pm.bgimg &&
      this.j_image.setStyle(
        "background-image",
        $.convertBackgroundImage(pm.bgimg, "image")
      );
    pm.bgimgpos &&
      this.j_image.setStyle(
        "background-position",
        $.convertBackgroundPosition(pm.bgimgpos)
      );
    pm.bgimgsize &&
      this.j_image.setStyle("background-size", $.convertLength(pm.bgimgsize));
    pm.bgimgrepeat &&
      this.j_image.setStyle("background-repeat", pm.bgimgrepeat);
    pm.bgopacity &&
      this.j_image.setStyle("opacity", $.convertOpacity(pm.bgopacity));
    ...
    pm.opentime &&
      (this.kag.tmp.remodal_opening_effect_time = pm.opentime + "ms");
    pm.closetime &&
      (this.kag.tmp.remodal_closing_effect_time = pm.closetime + "ms");
    pm.gotitle && (tyrano_lang.word.go_title = pm.gotitle);
    if (pm.okpos) {
      const insert_method =
        "left" === pm.okpos ? "insertBefore" : "insertAfter";
  ...
  changeButton(pm, is_ok) {
    ...
    pm.fontbold &&
      j_elm.setStyle("font-weight", $.convertFontWeight(pm.fontbold));
    pm.fontface && j_elm.setStyle("font-family", pm.fontface);
    pm.fontcolor && j_elm.setStyle("color", $.convertColor(pm.fontcolor));
    pm.img &&
      (this.css_map[`#${id}`]["background-image"] = $.convertBackgroundImage(
        pm.img,
        "image"
      ));
    if (pm.activeimg) {
      this.css_map[`#${id}:active`]["background-image"] =
        $.convertBackgroundImage(pm.activeimg, "image");
      this.css_map[`#${id}.active`]["background-image"] =
        $.convertBackgroundImage(pm.activeimg, "image");
    }
    if (pm.enterimg) {
      this.css_map[`#${id}:hover`]["background-image"] =
        $.convertBackgroundImage(pm.enterimg, "image");
      this.css_map[`#${id}.hover`]["background-image"] =
        $.convertBackgroundImage(pm.enterimg, "image");
    }
    pm.clickimg &&
      (this.css_map[`#${id}.clicked`]["background-image"] =
        $.convertBackgroundImage(pm.clickimg, "image"));
    this.updateCSS();
  ...
  createButton(pm) {
    ...
    j_img.on("mouseenter", () => {
      pm.enterimg &&
        ((pm.activeimg && 0 !== j_img.filter(":active").length) ||
          j_img.attr("src", pm.enterimg));
      pm.enterse && this.kag.playSound(pm.enterse);
    });
    j_img.on("mouseleave", () => {
      pm.activeimg
        ? clicked ||
          0 !== j_img.filter(":active").length ||
          j_img.attr("src", pm.img)
        : clicked || j_img.attr("src", pm.img);
      pm.leavese && this.kag.playSound(pm.leavese);
    });
    j_img.on("mousedown", () => {
    ...
    j_img.on("activeoff", () => {
      j_img.filter(":hover").length > 0
        ? (clicked = !0)
        : pm.activeimg && j_img.attr("src", pm.img);
    });
    j_img.on("click", () => {
      clicked = !0;
...
tyrano.plugin.kag.tag.mode_effect = {
  pm: { all: "", skip: "", auto: "", holdskip: "", stop: "", env: "all" },
  next: function (pm) {
    this.kag.ftag.nextOrder();
  },
...
tyrano.plugin.kag.tag.loading_log = {
  pm: { mintime: "", all: "", load: "", save: "", dottime: "" },
  initialized: !1,
  init() {
    if (!this.initialized) {
      this.initialized = !0;
  ...
  default_message_map: { preload: "Loading", save: "Saving" },
  start(pm) {
    this.init();
