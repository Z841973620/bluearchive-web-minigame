...
tyrano.plugin.kag.tag.showsave = {
  ...
  start: function (pm) {
    ...
    this.kag.menu.displaySave(void 0, () => {
...
tyrano.plugin.kag.tag.kanim = {
  ...
  start: function (pm) {
    const that = this,
      original_pm = $.extend({}, pm),
      j_targets = $.findAnimTargets(pm);
    if (0 === j_targets.length) {
      !1 !== pm._next && this.kag.ftag.nextOrder();
      return;
    }
...
tyrano.plugin.kag.tag.chara_new = {
  ...
  start: function (pm) {
    ...
    // 将 jname 和 logname 关联到 charas 中
    if ("" != pm.jname) {
      this.kag.stat.jcharas[pm.jname] = pm.name;
    }
    if ("" != pm.logname) {
      // 检查 logname
      this.kag.stat.jcharas[pm.logname] = pm.name; // 将 logname 关联到 charas 中
    }
...
tyrano.plugin.kag.tag.chara_show = {
  ...
  start: function (pm) {
    ...
    const preload_images = [],
      existing_chara = that.kag.chara.getCharaContainer(pm.name);
    if (existing_chara.get(0)) {
      existing_chara.stop(!0, !0);
      if ("none" != existing_chara.css("display")) {
        that.kag.ftag.nextOrder();
        return;
      }
    ...
    for (let key in chara_layer) {
      ...
      this.kag.chara.setFrameAnimation(
        cpm,
        key,
        current_part_id,
        j_img,
    ...
    "" != pm.reflect &&
      ("true" == pm.reflect ? (cpm.reflect = "true") : (cpm.reflect = "false"));
    ...
    this.kag.preloadAll(preload_images, () => {
      ...
      if ("true" == that.kag.stat.chara_pos_mode && "" === pm.left) {
        ...
        $(existing_chara_arr).each(function () {
          ...
          const half_width = Math.floor(parseInt(j_chara.css("width")) / 2),
            left = current_left - half_width;
          "false" == that.kag.stat.chara_anim
            ? j_chara
                .stop(!0, !0)
                .fadeTo(parseInt(that.kag.cutTimeWithSkip(pm.time)), 0, () => {
                  j_chara.css("left", left);
                  j_chara
                    .stop(!0, !0)
                    .fadeTo(pos_change_time, 1, on_animation_complete);
                })
            : j_chara
                .stop(!0, !0)
                .animate(
      ...
      "" != cpm.width && j_chara_base_img.css("width", cpm.width + "px");
      "" != cpm.height && j_chara_base_img.css("height", cpm.height + "px");
      "true" == cpm.reflect
        ? j_chara_root.addClass("reflect")
        : j_chara_root.removeClass("reflect");
      j_chara_root.stop(!0, !0).fadeIn({
      ...
      setTimeout(function () {
        const width = j_chara_base_img.css("width"),
          height = j_chara_base_img.css("height");
...
tyrano.plugin.kag.tag.chara_hide_all = {
  ...
  start: function (pm) {
    ...
    for (let key in charas)
      if (void 0 === charas[key].layer || charas[key].layer === pm.layer) {
        charas[key].is_show = "false";
        that.kag.chara.stopFrameAnimation(charas[key]);
      }
    if (img_obj.get(0)) {
      img_obj.stop(!0, !0).animate(
        { opacity: "hide" },
    ...
    if (0 != j_chara.length) {
      ...
      if ("" != storage_url)
        this.kag.preload(folder + storage_url, function () {
          ...
          j_old_face.length && j_old_face.remove();
          if ("0" != chara_time) {
            j_img.stop(!0, !0);
            ...
            j_img.addClass(
              "chara-mod-animation chara-mod-animation_" + pm.name
            );
  ...
  start: function (pm) {
    ...
    if (part_obj[pm.id])
      for (const key in part_obj[pm.id]) {
        pm[key] || (pm[key] = part_obj[pm.id][key]);
        part_obj.current_part_id === pm.id &&
          pm.lip_type &&
    ...
    if (pm.lip_image) {
      ...
      Array.isArray(pm.frame_image) ||
        (pm.frame_image = pm.frame_image.split(","));
      pm.frame_image = pm.frame_image.map((item) => item.trim());
      pm.lip_volume
        ? Array.isArray(pm.lip_volume) ||
          (pm.lip_volume = pm.lip_volume.split(","))
        : (pm.lip_volume = []);
      pm.lip_volume = pm.lip_volume.map((item) => parseInt(item));
      let prev_value;
      for (let i = 0; i < pm.frame_image.length; i++) {
        pm.lip_volume[i] ||
          (pm.lip_volume[i] = prev_value ? prev_value + 4 : 1);
      ...
      pm.frame_image.forEach((src, i) => {
        pm.frame_time[i]
          ...
          : (pm.frame_time[i] = prev_val || (0 === i ? [4e3, 8e3] : 40));
      });
    }
    if (pm.lip_se_buf) {
      pm.lip_se_buf = parseInt(pm.lip_se_buf);
      cpm.lipsync_bufs || (cpm.lipsync_bufs = []);
    ...
    "true" !== cpm.is_show || cpm._layer[pm.part].current_part_id !== pm.id
      ? this.kag.ftag.nextOrder()
...
tyrano.plugin.kag.tag.chara_part = {
  ...
  start: function (pm) {
    ...
    for (const part_id in pm) {
      ...
      if (part_map[part_id][state_id]) {
        if (
          ...
          if ("none" !== state_obj.storage) {
            const src = $.parseStorage(state_obj.storage, "fgimage");
            preload_srcs.push(src);
            const frame_srcs = this.kag.chara.getFrameAnimationSrcs(
              cpm,
              part_id,
              state_id
            );
    ...
    if (0 === Object.keys(target_map).length) {
      this.kag.ftag.nextOrder();
      return;
    }
      ...
      if (pm.time && "0" !== pm.time) {
        ...
        for (const part_id in target_map) {
          ...
          let j_img = j_chara.find(`.part.${part_id}`),
            j_img_last = j_img.eq(j_img.length - 1);
          ...
          j_img.stop(!0, !0).fadeTo(time, 0, () => {
            j_img.remove();
          });
          total_count += j_new_img.length;
          j_new_img.stop(!0, !0).fadeTo(time, 1, () => {
            completed_count++;
...
tyrano.plugin.kag.tag.web = {
  ...
  start: function (pm) {
    if (-1 == pm.url.indexOf("http")) window.open(pm.url);
      ...
      if (window.studio_api) window.studio_api.shell.openExternal(pm.url);
      else {
        require("electron").shell.openExternal(pm.url);
      }
    else $.isTyranoPlayer() ? $.openWebFromApp(pm.url): (window.location.replace(pm.url)); ;
    this.kag.ftag.nextOrder();
  },
};
